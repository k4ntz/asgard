#!/home/k4ntz/Documents/robotic/Asgard/asguard_env/bin/python
import sys
...
#FK Art6 Functions
    def FKMoveArt6(self): # En realidad esto no va asi, hay que calcular el movimiento acoplado. Proximamente.
        self.FKMoveAll()
        return
        if s0.isOpen():
            if self.G1MoveRadioButton.isChecked():
...
#FK Every Articulation Functions
    def FKMoveAll(self): # En realidad esto no va asi, hay que calcular el movimiento acoplado. Proximamente.
        if s0.isOpen():
            # if self.G1MoveRadioButton.isChecked():
            #     typeOfMovement="G1 "
            #     feedRate=" F" + str(self.FeedRateInput.value())
            ...
            message = typeOfMovement + \
                "A" + str(round(self.SpinBoxArt1.value() / 6, 2)) + \
                " B" + str(self.SpinBoxArt2.value()) + \
                " C" + str(self.SpinBoxArt2.value()) + \
                " D" + str(self.SpinBoxArt3.value()) + \
                " X" + str(self.SpinBoxArt4.value()) + \
                " Y" + str(art5 + art6) + \
                " Z" + str(-art5 + art6) + \
                feedRate
            messageToSend = message + "\n"
...
# Gripper Functions
    def MoveGripper(self): # En realidad esto no va asi, hay que calcular el movimiento acoplado. Proximamente.
        if s0.isOpen():
            message="M3 S" + str((255 / 20)*self.SpinBoxGripper.value())
            messageToSend = message + "\n"
            messageToConsole = ">>> " + message
            s0.write(messageToSend.encode('UTF-8'))
            self.ConsoleOutput.appendPlainText(messageToConsole)
...
# Serial Connection functions
    ...
    def updateFKPosDisplay(self,dataRead):
        ...
        self.FKCurrentPosValueArt1.setText(data[1][5:][:-2]+"deg")
        self.FKCurrentPosValueArt2.setText(data[2][:-2]+"deg")
        self.FKCurrentPosValueArt3.setText(data[4][:-2]+"deg")
        self.FKCurrentPosValueArt4.setText(data[5][:-2]+"deg")
        self.FKCurrentPosValueArt5.setText(data[6][:-2]+"deg")
        self.FKCurrentPosValueArt6.setText(data[7][:-2]+"deg")
